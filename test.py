...
SEED = 42
# A random seed is like a starting point for the random number generator.

# If you give the same seed every time (like 42), you’ll get the same random numbers every time.

os.environ['PYTHONHASHSEED'] = str(SEED)              # Fix Python hash randomization
os.environ['TF_DETERMINISTIC_OPS'] = '1'              # Make TensorFlow operations deterministic
random.seed(SEED)                                     # Fix random from Python’s random module
np.random.seed(SEED)                                  # Fix NumPy randomness
tf.random.set_seed(SEED)                              # Fix TensorFlow randomness

# Together, this makes the model give the same predictions every time — useful for testing, debugging, and fair comparisons."
